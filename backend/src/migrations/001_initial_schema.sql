-- =============================================
-- ESNAF-KURYE LOJİSTİK KÖPRÜSÜ - VERİTABANI ŞEMASI
-- Migration 001: İlk şema (idempotent)
-- =============================================
...
DO $$ BEGIN CREATE TYPE kurye_durum AS ENUM ('musait', 'mesgul', 'cevrimdisi'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE arac_tipi AS ENUM ('motorsiklet', 'bisiklet', 'otomobil', 'yaya'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE cagri_durum AS ENUM ('beklemede', 'atandi', 'teslim_alindi', 'teslimde', 'tamamlandi', 'iptal'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE odeme_yontemi AS ENUM ('nakit', 'sanal_pos'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE odeme_durum AS ENUM ('beklemede', 'dogrulandi', 'iptal'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE kullanici_rol AS ENUM ('esnaf', 'kurye'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
BEGIN
    ...
    SET ortalama_puan = (
        ...
        FROM puanlamalar p
        WHERE p.degerlendirilen_id = NEW.degerlendirilen_id
    )
